<template>
  <div class="space-y-6">
    ...
    <div v-else-if="torneos.length === 0" class="text-center py-8 bg-amber-50 rounded-lg border border-amber-200">
      <p class="text-amber-700 font-medium">No hay torneos disponibles para este equipo</p>
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      >
        <div class="flex items-center justify-between mb-4">
          <div>
            <h4 class="text-lg font-semibold text-slate-900">{{ torneo.nombre }}</h4>
            <p class="text-sm text-slate-600">{{ torneo.descripcion || 'Sin descripción' }}</p>
          </div>
          <label class="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span class="text-sm font-medium text-slate-700">Asignar</span>
          </label>
        </div>
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <h5 class="text-sm font-semibold text-slate-700">Asignaciones del jugador</h5>
            <button
              type="button"
          ...
          <div v-if="getTorneoAssignments(torneo.torneoId).length === 0" class="text-center py-4 bg-slate-50 rounded-lg">
            <p class="text-sm text-slate-500">No hay asignaciones. Haz clic en "Agregar" para comenzar.</p>
          </div>

          <div v-else class="space-y-3">
            <div
              v-for="(assignment, index) in getTorneoAssignments(torneo.torneoId)"
              ...
              <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                <div class="md:col-span-3">
                  ...
                  >
                    ...
                    <option
                      v-for="rama in getRamasPorTorneo(torneo.torneoId)"
                      :key="rama.ramaId"
                      :value="rama.ramaId"
                    >
                      {{ rama.nombre }}
                ...
                <div class="md:col-span-3">
                  ...
                  >
                    <option value="">Seleccionar</option>
                    <option
                      v-for="categoria in getCategoriasPorTorneo(torneo.torneoId, assignment.ramaId)"
                ...
                <div class="md:col-span-2">
                  ...
                  <select
                    v-model.number="assignment.posicionTipoDeporteId"
                    required
                    class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm"
                  >
                    <option value="">Seleccionar</option>
                ...
                <div class="md:col-span-2">
                  <button
                    type="button"
                    @click="removeAssignment(torneo.torneoId, index)"
...
const { obtenerTorneosPorEquipo, selectRamasPorEquipo, selectCategoriasPorEquipo, selectPosicionesTipoTorneo } = useEquipoService()
...
const isTorneoSelected = (torneoId: number): boolean => {
  return torneoAssignments.value[torneoId] !== undefined && torneoAssignments.value[torneoId].length > 0
}
...
const getCategoriasPorTorneo = (torneoId: number, ramaId?: number | null): CategoriaEquipo[] => {
...
const loadTorneos = async () => {
  ...
  try {
    ...
    torneos.value = torneosData.map(t => ({
      torneoId: t.torneoId,
      nombre: t.nombre,
...
const loadPosiciones = async () => {
  ...
  try {
    ...
    const posicionesData = await selectPosicionesTipoTorneo(props.tipoDeporteId)
...
const canSubmit = computed(() => {
  ...
  return selectedTorneos.every(torneoId => {
    ...
    return assignments && assignments.length > 0 && assignments.every(a => 
      a.ramaId !== null && 
      a.categoriaId !== null && 
...
const handleSubmit = async () => {
  ...
  const jugadorTorneo = Object.keys(torneoAssignments.value).map(torneoId => {
    ...
    return {
      ...
      jug: assignments.map(a => ({
        ...
        posicionTipoDeporteId: String(a.posicionTipoDeporteId ?? ''),
        num: String(a.num ?? ''),
...
watch(() => torneoAssignments.value, (newVal) => {
